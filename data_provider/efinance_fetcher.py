...
class EfinanceFetcher(BaseFetcher):
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        """
        获取实时行情数据（直接请求东财接口，绕过 efinance 库）
        """
        ...
        try:
            # 1. 识别市场代码
            market_id = "1" if str(stock_code).startswith("6") else "0"
            ...
            params = {
                ...
                # 扩展字段列表
                "fields": "f43,f58,f168,f50,f170,f46,f44,f45,f47,f48,f60," "f162,f167,f116,f117,f174,f175",  # 新增字段
                # f162: 市盈率(动态)
                # f167: 市净率
                # f116: 总市值
                # f117: 流通市值
            ...
            resp = requests.get(
                ...
                params=params,
                headers=headers,
                proxies={"http": None, "https": None},  # 关键：强制禁用代理
                timeout=5,
            )

            if resp.status_code != 200:
                logger.warning(f"[{stock_code}] 请求被拒绝，状态码: {resp.status_code}")
            ...
            if not data_json.get("data"):
                logger.warning(f"[{stock_code}] 接口返回空数据")
                return None

            data = data_json["data"]

            # 5. 构建 UnifiedRealtimeQuote 对象
            quote = UnifiedRealtimeQuote(
                ...
                change_amount=safe_float(data.get("f60")),
                ...
                # 新增字段
                pe_ratio=safe_float(data.get("f162")),  # 市盈率(动态)
                pb_ratio=safe_float(data.get("f167")),  # 市净率
                total_mv=safe_float(data.get("f116")),  # 总市值(单位：元)
                circ_mv=safe_float(data.get("f117")),  # 流通市值(单位：元)
                high_52w=safe_float(data.get("f174")),  # 52周最高
                low_52w=safe_float(data.get("f175")),  # 52周最低
                change_60d=None,  # 暂时设为 None，后续用历史数据计算
            )

            circuit_breaker.record_success(source_key)
            logger.info(
                f"[实时行情-eastmoney_direct] {stock_code} {quote.name}: "
