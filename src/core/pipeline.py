...
class StockAnalysisPipeline:
    ...
    def _enhance_context(
        ...
        if realtime_quote:
            # 【新增】计算60日涨跌幅
            change_60d = None
            if "raw_data" in context and context["raw_data"]:
                try:
                    import pandas as pd
                    ...
                    if len(df) >= 60:
                        ...
                        # 计算涨跌幅：(当前价 - 60日前价) / 60日前价 * 100
                        change_60d = round((current_price - price_60d_ago) / price_60d_ago * 100, 2)
                        logger.debug(
                            f"计算60日涨跌幅: 当前价={current_price}, 60日前={price_60d_ago}, 涨跌幅={change_60d}%"
                        )
                    else:
                        logger.debug(f"历史数据不足60天，无法计算60日涨跌幅（当前{len(df)}天）")
                except Exception as e:
                    logger.warning(f"计算60日涨跌幅失败: {e}")
            ...
            enhanced["realtime"] = {
                ...
                "change_60d": change_60d,  # 【新增】使用计算的60日涨跌幅
            ...
            if self.notifier.is_available():
                ...
                for channel in channels:
                    ...
                    if channel == NotificationChannel.FEISHU:
                        ...
                        non_wechat_success = self.notifier.send_to_showdoc(report) or non_wechat_success
                    elif channel == NotificationChannel.WECHAT_MP_DRAFT:
                        non_wechat_success = self.notifier.send_to_wechat_mp_draft(report) or non_wechat_success
                    else:
