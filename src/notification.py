...
class NotificationService:
    ...
    def __init__(self, source_message: Optional[BotMessage] = None):
        ...
        # ShowDoc 配置
        self._showdoc_url = config.showdoc_push_url
        ...
        self._wechat_mp_client: Optional[WechatMPDraftClient] = None
    ...
    def _detect_all_channels(self) -> List[NotificationChannel]:
        ...
        if self._is_wechat_mp_configured():
            channels.append(NotificationChannel.WECHAT_MP_DRAFT)
    ...
    def _has_context_channel(self) -> bool:
        ...
        return self._extract_dingtalk_session_webhook() is not None or self._extract_feishu_reply_info() is not None
    ...
    def send_to_showdoc(self, content: str, title: Optional[str] = None) -> bool:
        """
        推送消息到 ShowDoc (原 wechat_api.py 功能)
        """
        if not self._showdoc_url:
            logger.warning("ShowDoc 推送 URL 未配置，跳过推送")
            return False
        # 如果没有提供标题，自动生成
        if title is None:
    ...
    def send_to_wechat_mp_draft(self, content: str, title: Optional[str] = None) -> bool:
        """
        上传分析报告到微信公众号草稿箱

        流程：
        1. 获取 access_token
        2. 上传封面图获取 media_id（如果配置了封面图）
        3. 将 Markdown 转换为微信公众号兼容的 HTML
        4. 创建草稿
        ...
        if not self._is_wechat_mp_configured():
            logger.warning("微信公众号草稿箱配置不完整，跳过推送")
            return False

        # 初始化客户端（懒加载）
        if self._wechat_mp_client is None:
            self._wechat_mp_client = WechatMPDraftClient(
        ...
        try:
            # 上传封面图
            cover_path = self._wechat_mp_config.get("cover_path")
            thumb_media_id = None
            ...
            if not thumb_media_id:
                logger.error("无法获取封面图 media_id，请配置 WECHAT_MP_COVER_PATH 或放置默认封面图")
                return False
            ...
            # 创建草稿
            media_id = self._wechat_mp_client.add_draft(
                title=title,
            ...
            if media_id:
                logger.info(f"微信公众号草稿创建成功，请登录公众号后台手动发布")
                return True
            else:
    ...
    def _markdown_to_html(self, markdown_text: str) -> str:
        ...
        html_content = markdown2.markdown(
            markdown_text, extras=["tables", "fenced-code-blocks", "break-on-newline", "cuddled-lists"]
        )
    ...
    def _send_discord_webhook(self, content: str) -> bool:
        ...
        """
        try:
            payload = {"content": content, "username": "A股分析机器人", "avatar_url": "https://picsum.photos/200"}

            response = requests.post(self._discord_config["webhook_url"], json=payload, timeout=10)
    ...
    def _send_discord_bot(self, content: str) -> bool:
        ...
        """
        try:
            headers = {"Authorization": f'Bot {self._discord_config["bot_token"]}', "Content-Type": "application/json"}
    ...
    def send(self, content: str) -> bool:
        ...
        for channel in self._available_channels:
            ...
            try:
                ...
                elif channel == NotificationChannel.SHOWDOC:
                    result = self.send_to_showdoc(content)
                elif channel == NotificationChannel.WECHAT_MP_DRAFT:
                    result = self.send_to_wechat_mp_draft(content)
                else:
